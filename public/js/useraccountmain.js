...
//This populates the UserProfileForm, found in the userutil.html (or userutil/index.html)
  populateForm() {
    ...
    return this.fetch('http://www.patric.local:3000' + '/user/', fetchData)
  ...
  updateUserPrefs() {
    ...
    return this.fetch('http://www.patric.local:3000' + '/user/' + this.uid, fetchData)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      document.getElementsByClassName('UserProfileForm')[0].style.display = 'none';
...
// this is only the initial request to change the email address from the User Prefs page
  changeUserEmail() {
    ...
    return this.fetch('http://www.patric.local:3000' + '/auth/changeemail', fetchData)
  .then((response) => response.json())
  .then((data) => {
    if (data.message) {
