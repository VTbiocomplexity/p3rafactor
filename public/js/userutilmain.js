...
class User_ {
  ...
  updateUser(evt) {
    ...
    return runFetch(fetchClient, 'http://www.patric.local:3000', '/auth/validemail', fetchData);
  }
  ...
  resetPasswd(evt) {
    ...
    return runFetch(fetchClient, 'http://www.patric.local:3000', '/auth/passwdreset', fetchData);
  }
  ...
  verifyChangeEmail(evt) {
    ...
    return runFetch(fetchClient, 'http://www.patric.local:3000', '/auth/updateemail', fetchData);
  //   .then((response) => response.json())
  //   .then((data) => {
  //     if (data.message) {
  //       let messagediv = document.getElementsByClassName('loginerror')[0];
